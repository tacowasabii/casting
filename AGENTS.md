...
- **NAS SMB 유령 디렉터리 대응 (제거 금지)**: 기본 `.next/dev/types` 경로가 SMB
  ...
  문제가 있어 `distDir: ".next-dist"`(next.config.ts) + `scripts/fs-nas-shim.cjs`
  (fs 재시도/삭제차단 shim, next.config.ts에서 주입)를 적용했다. tsconfig include도
  `.next-dist/...` 기준. 같은 증상이 재발하면 distDir을 또 다른 이름으로 바꾸면 된다.
